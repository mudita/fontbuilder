...
bool CArrayExporter::Export(QByteArray &out)
{
    ...
    out.append( QString("#ifndef ") + QString("%1%2_%3_H\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toUpper().toUtf8());
 ...
 */
    ...
    out.append( QString("\tstatic const struct font_info fi_%1%2_%3 = { \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
        + QString("\t\t.face=\"%1\", \\\n").arg(cfg->family())
        + QString("\t\t.size=%1, \\\n").arg(cfg->size())
        + QString("\t\t.bold=%1, \\\n").arg(cfg->bold() ? 1 : 0)
        + QString("\t\t.italic=%1, \\\n").arg(cfg->italic() ? 1 : 0)
 ...
 */
    ...
    out.append( QString("\tstatic const struct font_common fc_%1%2_%3 = { \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
 ...
 */
    ...
    out.append( QString("\tstatic const struct font_page fp_%1%2_%3 = { \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
        + QString("\t\t.id=%1, \\\n").arg(0)
        + QString("\t\t.file=\"%1\" \\\n};").arg(texFilename())
        .toUtf8()).append('\n');
 ...
 */
    ...
    out.append( QString("\tstatic const struct font_char fch_%1%2_%3[] = { \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower().toUtf8());
    ...
    //const uint16_t CALIBRI_REGULAR_24_CHARSNUM = sizeof(fch_calibri_regular_24) / sizeof(fch_calibri_regular_24[0]);
...
/*
    ...
    out.append( QString("static const struct font_kerning fk_%1%2_%3[] = {\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toUtf8());
...
*/
    ...
    out.append( QString("const struct font f_%1%2_%3 = { \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower().toUtf8());
    out.append( QString("\t.fi=&fi_%1%2_%3, \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
            + QString("\t.fc=&fc_%1%2_%3, \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
            + QString("\t.fp=&fp_%1%2_%3, \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
            + QString("\t.fch=fch_%1%2_%3, \\\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
            //+ QString("\t.fk=fk_%1%2_%3 \\").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower()
            + QString("\t.fd=font_data_%1 \\").arg(filename)
            .toUtf8()).append('\n');
    out.append( QString("};\n\n").toUtf8());

    out.append( QString("#endif\n\n").toUtf8());

    out.append( QString("extern const struct font f_%1%2_%3;\n").arg(cfg->family()).arg(tmp).arg(cfg->size()).toLower().toUtf8());
    //out.append( QString("extern const uint16_t ").toUtf8() + QString("%1_SIZE;\n\n").arg(filename).toUpper().toUtf8());
